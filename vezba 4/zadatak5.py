...
def zad():
    ...
    for i in range(len(korisnici)):
        ...
        podaci_o_korisnicima = korisnik.strip().split("|")

        racun = racuni[i]
        cene = racun.strip().split("|")

        ukupna_cena = 0
        for cena in cene:
            ukupna_cena = ukupna_cena + float(cena)

        prosecna_cena = ukupna_cena / len(cene)

        stat = f"{podaci_o_korisnicima[0]}|{ukupna_cena:.2f}|{prosecna_cena:.2f}\n"
        g.write(stat)
